...
<Link href={`/post/${post.slug}`}>
    {post.title}
        ...
        <span className="align-middle">{moment(post.createdAt).format('MMM DD, YYYY')}</span>
            ...
            <p className='px-4 mb-8 text-lg font-normal text-center text-gray-700 lg:p-2'>{post.excerpt}</p>
        <div className='text-center'>
            <Link href={`/post/${post.slug}`}>
                <span className='inline-block px-8 py-2 text-lg font-medium text-white transition duration-500 transform bg-pink-600 rounded-full cursor-pointer hover:-translate-y-1'>
                    Continue Reading
                </span>
            </Link>
        </div>
